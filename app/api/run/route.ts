...
import { runSuperAgent } from "@/lib/runtime/superAgent";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const superAgentOutput = runSuperAgent(plan);

    const selectedModel = pickModel({
      plan,
      preflight,
      preferredModel: superAgentOutput.bot_drafts[0]?.preferred_model
    });

    const botPackage = buildBotPackage(
      {
        ...plan,
        normalized_task: superAgentOutput.normalized_task
      },
      selectedModel
    );
    ...
    const workerOutput = await runWorker({
      taskSpec: superAgentOutput.normalized_task,
      botPackage,
      artifactsDir,
    ...
    const audits: AuditReport[] = [
      runSpecAuditor(superAgentOutput.normalized_task, botPackage, workerOutput),
      runQualityAuditor(workerOutput),
      runRiskAuditor(superAgentOutput.normalized_task, workerOutput),
      runBudgetAuditor(superAgentOutput.normalized_task, preflight, botPackage)
    ];
    ...
    const superAgentPath = await writeArtifact(artifactsDir, "super_agent.json", superAgentOutput);
    ...
    return Response.json({
      ...
      artifact_refs: {
        ...
        super_agent: path.relative(process.cwd(), superAgentPath),
